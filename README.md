...
| 🛡️ | **更新即回退** | 升级前自动备份，冒烟测试失败即刻回滚——不会"变砖"；兼容矩阵命中破坏性版本时弹黄色警告 |
| 🌐 | **国内网络友好** | 内核与插件全部走 npmmirror 镜像；GitHub 源插件可一键挂镜像加速 |
| 🐋 | **四款内置插件** | 插件市场、小鲸鱼余额挂件、用量统计面板、**对话导航条**——开箱即用，面板一键开关 |
| 🖥️ | **原生桌面体验** | 托盘驻留（tooltip 显示 dsh 版本/端口）、关闭最小化、单实例、Dock 重新激活、崩溃自恢复、窗口位置记忆、开机自启 |
| 🔗 | **深链与文件关联** | `.dshpreset` 双击即导入向导；`dsh-cockpit://import?path=...` 深链一键导入 |
| 🔔 | **系统通知** | dsh 就绪/崩溃/更新可用/导入结果均有系统通知；`Cmd+Shift+P` 全局快捷键唤出控制面板 |
...
| 对话导航条（本项目维护，源码见 `plugins/`） | `dsh-conversation-navigator` | 长对话右侧刻度条：悬停预览、点击跳转 |

- 控制面板「插件」区提供**热开关**（写入官方补丁层，HMR 约 1 秒生效，无需重启）；
- 全部走 npm（npmmirror）安装，**国内无需访问 GitHub**；
- 导航条插件随包内置源码，发布 npm 后新用户自动安装（存量用户面板可一键安装）。
...
- **更新策略（双通道）**：内核随 npm 更新（启动延迟+抖动检查、三选一、跳过持久化、兼容矩阵黄色警告）；外壳随 GitHub Release 更新（`tauri-plugin-updater`，面板「检查外壳更新」按钮，签名校验）
...
- **UI 注入清单**：版本化 JS 注入层（`trace-zh` 轨迹面板字段汉化、`dsh-cockpit-bridge` 白名单 IPC 桥；`--safe-mode` 整体关闭、失败静默降级）
- **宿主扩展**：系统通知（就绪/崩溃/更新可用）、全局快捷键 `Cmd+Shift+P` 打开控制面板、`window.__dshCockpit` 桥（开日志目录/导入预设/重启服务/读版本）
- **对话导航条插件**（`plugins/dsh-conversation-navigator`）：会话右侧刻度条，悬停预览摘要、点击平滑跳转、消息数<15 自动隐藏；随 dsh 插件体系安装/热开关
- **安全模式**：`--safe-mode` 以仅含官方核心 bundle 的隔离 profile 启动；托盘可直接「以安全模式重启」；面板 banner 显示已停用插件清单
- **窗口恢复与记忆**：主窗口意外销毁后自动重建（限频防抖）；大小/位置跨启动记忆；开机自启面板开关
- **导航守卫**：仅允许加载客户端页面与本机服务，外部链接一律交系统浏览器
- **环境注入**：解析用户登录 shell 的完整环境并注入 dsh 子进程（Finder 启动也能找到 Homebrew / mise 等工具）
- **体验细节**：渐变流光加载进度条 + 可折叠日志、托盘 tooltip 实时显示 dsh 版本与端口、右键 Inspect 全面屏蔽
